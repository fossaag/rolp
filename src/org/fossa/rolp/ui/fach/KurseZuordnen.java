...
public class KurseZuordnen extends FossaWindow implements Button.ClickListener, ItemClickListener, CloseListener {
	...
	private Button kursHinzufuegenButton = new Button ("Hinzufügen", (ClickListener) this);
		...
		if (source == windowCloseButton) {
			...
			if (fach != null) {
				if (ZuordnungFachSchuelerContainer.getAllSchuelerOfFach(fach.getPojo()).size() > 0) {
					getWindow().showNotification("Der Kurs kann nicht gelöscht werden, da ihm bereits Schüler zugeordnet sind.");
					return;
				}
				...
				confirmDeleteFach = new FossaBooleanDialog(app, " - Bestätigung - ", "Möchten Sie den Kurs '" + fach.getFachbezeichnung() + "' wirklich entgültig entfernen?", "Ja", "Nein");
		...
		if (source == confirmDeleteFach) {
			if (confirmDeleteFach.getDecision()) {
				...
				if (fachlehrer1 != null) {
					...
					lehrerblog.setEreignis("Der von Ihnen unterrichtete Kurs '" + fach.getFachbezeichnung() + "' wurde gelöscht.");
